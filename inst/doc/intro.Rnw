...
Of course, some operations fundamentally alter the data in
such a way that re-conversion is either impossible or
less that straightforward. In these cases the are additional tools
in the package to help in the process.
...
present in {\sf xts} objects. In addition, most methods that can accept
{\sf zoo} or {\sf matrix} objects will simply work as expected.

\noindent
The most noticably difference in the behavior of {\sf xts} objects
will be apparent in the use of the {\quote [} operator. Using
...
\noindent
The format must be left-specified with respect to the standard ISO
time format - CCYY-MM-DD HH:MM:SS.  This mean that for one
...
to include the full year, month and day - e.g. "2007-01-01 8'.
