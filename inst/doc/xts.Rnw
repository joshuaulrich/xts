...
{\tt as.xts}, or to create a new object with the {\tt xts} constructor.
...
A quick tour of some of the methods leveraged to {\tt xts}
...
and attributes when the function you need doesn't seem
to care.
...
to adopt the convention you do (\textbf{bad}), or allow for
multiple object classes by relying on internal
code to convert to one consistent type (\textbf{good}).
...
\section{Conclusion}
The {\tt xts} package offers both R developers and users
