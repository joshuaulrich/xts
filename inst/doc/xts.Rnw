...
{\tt as.xts}, or create a new object with the {\tt xts} constructor.
...
values for indexing,
can also be a character string, a time-based object, or vector of either.
The format must left-specified with respect to the standard ISO 
...
or the recommended ISO range seperator ``/''.
...
\subsection{Restoring the original class - {\tt reclass} \& {\tt use.reclass}}
...
are expecting --- the principal of least surprise.
...
for developers will be covered later in the Developing with
xts section of this document.

A user friendly interface of this \emph{reclass} functionality, though
implicit, is available in the {\tt use.reclass} function.
...
\subsubsection*{Letting xts handle the details: {\tt use.reclass}}
...
that meets these criteria in {\tt use.reclass} will
result in an
...
(zf <- filter(z, 0.2))
class(zf)
# using use.reclass, the zoo class is preserved
(zf <- use.reclass(filter(z, 0.2)))
class(zf)
@

The {\tt use.reclass} function is still a bit experimental, and will
certainly improve in time, but for now provides at least
an alternative option to maintain your object's class
and attributes
when the function you need doesn't.
...
inevitibly come from the massive increase in code.
...
by the xts package allows you the developer an opportunity
...
convert, the second is any additional arguments to
the {\tt as.xts} constructor that is called internally
(ignore this for the most part), and the third is
...
halted at this point, or continue onward. Accepting
either a logical value --- the result of which is
...
to preserve an untouched copy for use in the reclass function.
...
must be an untouched object returned from the
...
not {\tt NULL}. Essentially if the object meant to be
...
It is also likely that your function does not return
an obejct that is expected to be used in the same context
as the original object.  This would be the case for functions
that summarize an object, or return some additional statistics.

For functions that do not need the reclass functionality,
...
even confusing to the end user (this may depend on your users).
...
handled directly by the user (without the assistance
...
all the tools of xts and zoo.

\section{Conclusion}
The {\tt xts} package offers the R developer and user
...
true time-based indexing allows for code to
...
affords developers an opprtunity to design applications
...
within the package.  An effort to provide external based disk and
