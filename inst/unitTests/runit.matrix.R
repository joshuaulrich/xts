...
# The reclass tests seem to fail due to different attributes
no_test.matrix_reclass <- function() {
  checkIdentical(sample.matrix,reclass(as.xts(sample.matrix)))
}
no_test.matrix_reclass_subset_reclass_j1 <- function() {
  checkIdentical(sample.matrix[,1],reclass(as.xts(sample.matrix))[,1])
}
