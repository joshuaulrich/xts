...
Note that the default for behavior for `addLegend` is to add to the 
current panel.
```{r fig.keep='last'}
plot(sample.xts[,"Close"])
...
lines(TTR::SMA(sample.xts[,"Close"], n = 60), on=3, col="darkred", lty=2)
# draw a line on panel 2
lines(TTR::SMA(sample.xts[,"Close"], n = 25), on=2, col="darkblue")
# draw a line on the current panel
lines(TTR::SMA(sample.xts[,"Close"], n = 10), col="purple", lty=2)
...
addEventLines(c("2007-03-20","2007-05-28"), c("foo", "bar"), 
              offset=.4, pos=2, , srt=90, cex=1.5)
addEventLines(c("2007-03-20","2007-05-28"), c("foo", "bar"), 
              offset=.4, pos=4, , srt=-90, cex=1.8)


```

A new panel is drawn if `on=NA`. Event lines can
be drawn on a specific panel by specifying an integer corresponding to the 
desired panel for `on`.
```{r}
plot(sample.xts[,"Close"])
# add a new panel
lines(TTR::SMA(sample.xts[,"Close"], n = 20), on=NA, col="blue")
# add a new panel
lines(TTR::SMA(sample.xts[,"Close"], n = 50), on=NA, col="red")
addEventLines(c("2007-03-20","2007-05-28"), c("foo", "bar"), on=1:3,
              offset=.4, pos=2, srt=90, cex=1.5, col="red")
```
