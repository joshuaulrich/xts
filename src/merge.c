                   ...
                   SEXP coerce)
{
  ...
  */
  ...
  if( !Rf_asInteger(isXts(y)) ) {
    ...
    INTEGER(len_xindex)[0] = xlength(xindex);
    SETCAR(t, len_xindex);
...
SEXP mergeXts (SEXP args) // mergeXts {{{
{
  ...
  if(n > 2 || leading_non_xts) { /*args != R_NilValue) {*/
    ...
    index_len = xlength(GET_xtsIndex(_INDEX));
